...
class Particle:
  ...
  def __init__(self, x, y, vx, vy, ax, ay, r, id, color):
    self.x = x
    self.y = y
    self.vx = vx
    self.vy = vy
...
def createParticle(mouse_pos, particles, id_count):
  ...
  id_count += 1
  p = Particle(mouse_pos[0], mouse_pos[1], 0, 0, 0, 0, 40, id_count, random.choice(colors))
  particles.append(p)
...
def createParticlesInit(particles, id_count):
  ...
  for i in range(3):
    ...
    p = Particle(random.randint(XBORDER[0],XBORDER[1]), random.randint(YBORDER[0],YBORDER[1]), xv, yv, 0, 0, 20, id_count,random.choice(colors))
    particles.append(p)
